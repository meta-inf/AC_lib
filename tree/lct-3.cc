/*
 Suggested version by the end of March 2013
 The correctness of the previous two versions is not guarenteed.
 */
...
inline void cut (node *a, node *b) { // cut b and its father in the tree rooted by a
	evert(a);
	access(b); splay(b); b->c[0]->f = node::nil; b->c[0] = node::nil; b->update();
}
